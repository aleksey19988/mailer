$("#api-request-form").on("submit", function(event){
    ...
    $.ajax({
        ...
        error: function (data) {
            $('.spinner-container').css('display', 'none');
            $('.api-response-error-content').html(`Ошибочка: ${data.result}`);
            $('.api-response-error-alert').show();
        },
        success: function(data){
            $('.api-response-success-content').html(`<p><strong>Вопрос:</strong> ${data.requestMessage}</p><p><strong>Ответ:</strong> ${data.responseMessage}</p>`);
            $('.api-response-success-alert').show();
        }
    });
});
